...
class StudentEvaluationInput(BaseModel):
    ...
    detected_strengths: list[str] = Field(default_factory=list, description="Strengths parsed from resume")
    detected_challenges: list[str] = Field(default_factory=list, description="Weaknesses or challenges parsed from resume")
...
class CategorizedUniversity(BaseModel):
    ...
    requirements: dict[str, str] = Field(
        ...
        description="Official university admission criteria: min CGPA, GRE requirement, English proficiency, prerequisite courses"
    )
    tailored_advice: str = Field(
        default="",
        description="Actionable guidance on how to optimize SOP, resume, or credentials for this specific university"
    )
